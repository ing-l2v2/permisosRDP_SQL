...
foreach ($mail in $items) {
  ...
  # Write-Host "CC: $($mail.CC | ForEach-Object { $_.Address } -join ', ')"
  Write-Host "CC: $ccTexto"

  # 1) FILTRO DE APROBADOR
  # Buscar el correo previo (solicitud)
  $correoAprobador = $mail.SenderEmailAddress.ToLower()
  $nombreAprobador = $mail.SenderName.ToLower()
  # Confirmar que el remitente sea un aprobador permitido
  if ($correoAprobador -notin $aprobadoresValidos) {
    Write-Host "1. X DESCARTADO: No es aprobador valido"
    continue  # no es valido
  }
  Write-Host "1. Aprobador valido"

  <#
  if (-not ($keywordsAprobacion | Where-Object { $bodyLower.Contains($_) })) {
    continue
    }
    #>
  ...
  if ($ccTexto.ToLower() -notmatch $ccRequerido) {
    Write-Host "2. X DESCARTADO: No tiene CC requerido"
    continue
  }
  Write-Host "2. CC valido ($ccTexto)"
  ...
  if (-not $esAprobacion) { 
    Write-Host "3. X DESCARTADO: No contiene palabras de aprobacion"
    continue 
  }
  Write-Host "3. Tiene palabras clave de aprobacion $bodyLower" 
  ...
  if (-not $lineaInicioHilo) {
    Write-Host "4. X DESCARTADO: No se detectó inicio del hilo"
    continue
  }
  ...
  if ($partes.Count -le 1) { 
    Write-Host "4. X DESCARTADO: No contiene hilo (no hay solicitud original)"
    continue 
  }
  Write-Host "4. Hilo detectado (Solicitud encontrada)"

  # El aprobador siempre es la parte antes del primer hilo
  $cuerpoAprobador = Limpia-Texto($partes[0])
  #>
  ...
  if (-not $matchCorreo.Success) {
    Write-Host "6. X DESCARTADO: No se detecto correo del solicitante"
    continue
  }  
  ...
  if (-not $matchNombre.Success) {
    Write-Host "7. X DESCARTADO: No se detecto NOMBRE DEL SOLICITANTE"
    continue
  }
  Write-Host "7. Nombre del solicitante detectado: $($matchNombre.Groups[1].Value.Trim())"
  ...
  if (-not $esSolicitudValida) { 
    Write-Host " 8. X DESCARTADO: Texto del solicitante NO contiene keywords"
    continue 
  }

  Write-Host "8. Solicitud valida"
  ...
  # Registrar en SQL
  $sql = "INSERT INTO solicitudes (fecha, fecha_solicitud, correo_solicitante, nombre_solicitante, correo_aprobador, nombre_aprobador, asunto, texto_solicitud, texto_aprobador) VALUES ('" +
  $mail.ReceivedTime.ToString("yyyy-MM-dd HH:mm:ss") + "','" +
  $fechaSolicitud.Replace("'", "''") + "','" +
  $correoSolicitante.Replace("'", "''") + "','" +
  $nombreSolicitante.Replace("'", "''") + "','" +
  $correoAprobador.Replace("'", "''") + "','" +
  $nombreAprobador.Replace("'", "''") + "','" +
  $mail.Subject.Replace("'", "''") + "','" +
  $textoSolicitud.Replace("'", "''") + "','" +
  $cuerpoAprobador.Replace("'", "''") + "');"  
  ...
  $contador++
  Write-Host "Procesando: $contador correos en estado valido"
}
